  ...
  const { sub, role, email } = jwtDecode(token);
    ...
    try {
      const { data } = await api.post(
        ...
        }legal-infrahive/district-court/search/advocate-name/`,
        { name, stage: stage || "BOTH", complexId },
        {
          headers: {
      ...
      const resultsArray = Array.isArray(data)
        ? data.flatMap((court, courtIndex) =>
            court.cases.map((caseItem, caseIndex) => ({
              id: `${court.name}-${caseItem.cnr}-${courtIndex}-${caseIndex}`,
              cnr: caseItem.cnr,
              title: caseItem.title,
              caseNumber: caseItem.caseNumber,
              type: caseItem.type,
              decisionDate: caseItem.dateOfDecision,
              name: court.name,
              advocateName: caseItem.advocateName || caseItem.advocate || name,
                        ...
                        {isOwner && (
                          <td className="px-4 py-3 text-sm text-gray-900">
                            <button
                              ...
                              onClick={() =>
                                handleFollowCase(caseItem, index, caseItem.name)
                              }
      ...
      {showCaseDetails && (
        <DistrictCaseDetailsModal
          ...
          handleFollowCase={(caseData) =>
            handleFollowCase(caseData, "modal", caseData.name)
          }
